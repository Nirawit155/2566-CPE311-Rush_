...
char Num1[4], Num2[4];
int Dice_count, random, idx, Position, temp = 0;
uint8_t TxT_S[] = "S";		//Start
uint8_t TxT_D[] = "Dx";		//Dice x
uint8_t TxT_Y[] = "Y";		//Yes
uint8_t TxT_N[] = "N";		//No
...
int main(void)
{
  SystemClock_Config();
	...
	temp = 0;
	Usart_SendString(TxT_S, sizeof(TxT_S));		//Start
...
void GameRun(int Player)
{
	if(P[Player-1].Position > 1 && P[Player-1].Position < 5)
		Position = P[Player-1].Position - 1;
...
void BuyOrNot(int Player)
{
	...
	while(1)
	{
		...
		else if(Player == 2)
		{
			...
			else if(LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_4) == SET)
			{
				while(LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_4) == SET);
				Usart_SendString(TxT_N, sizeof(TxT_N));
				break;
			}
		}
...
void WaitReady(void)
{
	while(1)
	{
		...
		rec[0] = LL_USART_ReceiveData8(USART2);
		if(rec[0] == 'R')
			break;
	}
}
...
void Dice(int Player)
{
	...
	while(Dice_count < 6)
	{
		if((LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0) == SET || LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_1) == SET) && Player == 1)
			Dice_count = 1;
		else if((LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_4) == SET || LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_5) == SET) && Player == 2)
			Dice_count = 1;
...
void EXTI15_10_IRQHandler(void)
{
	...
	{
		P[0].Money = 100, P[0].Position = 0;
		P[1].Money = 100, P[0].Position = 0;
		for(int x=0;x<12;x++)
			M[x].Owner = 0;
		Usart_SendString(TxT_S, sizeof(TxT_S));
		temp = 1;
...
void Gpio_Config(void)
{
	...
	//Init ALL PIN for Matrix Switch
	GPIO_set.Mode = LL_GPIO_MODE_INPUT;
	GPIO_set.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
	GPIO_set.Speed = LL_GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_set.Pull = LL_GPIO_PULL_NO;
	GPIO_set.Pin = LL_GPIO_PIN_0 | LL_GPIO_PIN_1 | LL_GPIO_PIN_4 | LL_GPIO_PIN_5;
	LL_GPIO_Init(GPIOA, &GPIO_set);
